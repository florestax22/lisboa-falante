"use strict";
(() => {
  const S = {gps:null,manual:null,voice:false,voiceName:"",lastSpeech:"",watch:null,pendingPhone:"",texts:{},routeText:"",mapsReady:false,geocoder:null,directions:null,places:null,autoGpsTried:false};
  ...
  const boxes={commerce:"resultado-comercio",transport:"resultado-transportes",public:"resultado-utilidades",leisure:"resultado-lazer",emergency:"resultado-emergencia",free:"resultado-livre"};
  ...
  function loadMaps(){
    ...
    window.iniciarLisboaFalante=()=>{
      ...
      S.mapsReady=true;
      text("estado-servidor","Aplicação pronta. A tentar obter a localização atual.");
      if(!S.autoGpsTried){S.autoGpsTried=true;setTimeout(startGps,300)}
    };
  ...
  function geoError(e){
    if(e&&e.code===1)return "A localização foi recusada. Autoriza a localização no navegador ou escreve a partida.";
    if(e&&e.code===2)return "O dispositivo não conseguiu determinar a localização. Tenta novamente ou escreve a partida.";
    if(e&&e.code===3)return "A localização demorou demasiado. Tenta novamente.";
    return "Não foi possível obter a localização. Usa a partida escrita.";
  }
  async function updatePos(p){const c={lat:p.coords.latitude,lng:p.coords.longitude,accuracy:Math.round(p.coords.accuracy||0)};S.gps=c;try{const r=await reverseGeocode(c.lat,c.lng);Object.assign(S.gps,r);
      const msg="Localização atual: "+r.address+". Precisão aproximada: "+c.accuracy+" metros."+(r.distrito?"":" A localização parece estar fora do distrito de Lisboa.");
      text("estado-gps",msg);
      if(!S.gps.announced){S.gps.announced=true;speak(msg)}}catch(e){text("estado-gps","GPS ativo, mas não consegui obter o nome da rua: "+e.message)}}
  function startGps(){if(S.watch!==null){text("estado-gps","O GPS já está ativo e a atualizar a localização.");return;}if(!("geolocation" in navigator)){text("estado-gps","Este navegador não tem geolocalização. Usa a partida manual.");return}text("estado-gps","A iniciar GPS contínuo.");S.watch=navigator.geolocation.watchPosition(updatePos,e=>text("estado-gps",geoError(e)),{enableHighAccuracy:true,timeout:30000,maximumAge:5000});$("localizar").disabled=true;$("parar-gps").disabled=false}
  ...
  function textSearch(location,query){return new Promise(resolve=>S.places.textSearch({location,radius:5000,query},(r,status)=>resolve(status===google.maps.places.PlacesServiceStatus.OK?(r||[]):[])))}
  async function freeSearch(){
    const o=origin(),q=val("pesquisa-livre"),box=$("resultado-livre");
    if(!o){text("resultado-livre","Ainda não tenho localização. Autoriza o GPS ou confirma uma partida escrita.");return}
    if(!q){text("resultado-livre","Escreve o que queres procurar.");$("pesquisa-livre").focus();return}
    if(!S.mapsReady){text("resultado-livre","O serviço de mapas ainda não está pronto.");return}
    box.innerHTML="<p>A pesquisar "+esc(q)+".</p>";
    try{
      let ps=(await textSearch({lat:o.lat,lng:o.lng},q)).map(p=>({raw:p,distance:distanceMeters(o,{lat:p.geometry.location.lat(),lng:p.geometry.location.lng()})})).sort((a,b)=>a.distance-b.distance).slice(0,8);
      let html="<h3>Resultados para "+esc(q)+"</h3>",narr="Resultados para "+q+". ";
      if(!ps.length){html+="<p>Não encontrei resultados próximos.</p>";narr+="Não encontrei resultados próximos."}
      else ps.forEach((p,i)=>{const name=p.raw.name||"Local sem nome",address=p.raw.formatted_address||p.raw.vicinity||"Morada não indicada";narr+=(i+1)+". "+name+", "+address+", a cerca de "+distanceSpoken(p.distance)+". ";html+="<article><h3>"+esc((i+1)+". "+name)+"</h3><p>"+esc(address+". Distância aproximada: "+distanceSpoken(p.distance)+".")+"</p></article>"});
      S.texts.free=narr+safety();box.innerHTML=html;box.focus();
    }catch(e){box.innerHTML='<p class="erro">'+esc(e.message)+"</p>";S.texts.free="Não consegui pesquisar. "+e.message}
  }
  ...
  bind("procurar-livre",freeSearch);bind("narrar-livre",()=>narrate("free"));
  ...
  ["destino","origem-manual","pesquisa-livre"].forEach(id=>{const e=$(id);if(e)e.addEventListener("keydown",ev=>{if(ev.key!=="Enter")return;ev.preventDefault();if(id==="destino")route();else if(id==="origem-manual")manual();else freeSearch()})});
